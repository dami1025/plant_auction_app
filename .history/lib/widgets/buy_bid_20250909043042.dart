...
class _PlaceBidButtonState extends ConsumerState<PlaceBidButton> {
  ...
  Widget build(BuildContext context) {
    final bidSession = ref.watch(biddingSessionProvider(widget.docId));

    final now = DateTime.now();
    final lastBidTime = bidSession.lastBidTime ?? bidSession.startTime ?? now;
    final elapsed = now.difference(lastBidTime).inSeconds;

    BiddingSessionStatus status;
    if (bidSession.bidCount > 0) {
      status = elapsed >= 60 ? BiddingSessionStatus.ended : BiddingSessionStatus.active;
    } else {
      status = elapsed >= 120 ? BiddingSessionStatus.cancelled : BiddingSessionStatus.active;
    }

    final isActive = status == BiddingSessionStatus.active;
          ...
          actions: [
            ...
            ElevatedButton(
              ...
              onPressed: () async {
                ...
                await _placeBid(ctx, context, bid);
              },
              child: const Text(
                "Submit Bid",
  ...
  Future<void> _placeBid(BuildContext bidDialogContext, BuildContext parentContext, double bidAmount) async {
    ...
    try {
      ...
      if (loadingContext != null && mounted) Navigator.of(loadingContext!).pop();
      if (bidDialogContext.mounted) Navigator.of(bidDialogContext).pop();
      ...
      if (loadingContext != null && mounted) Navigator.of(loadingContext!).pop();
      if (parentContext.mounted) _showErrorSnackBar(parentContext, "Failed to place bid. Please try again.");
