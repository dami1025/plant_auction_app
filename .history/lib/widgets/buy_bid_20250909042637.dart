...
class _PlaceBidButtonState extends ConsumerState<PlaceBidButton> {
  ...
  void initState() {
    super.initState();
    // Rebuild every second to reflect timer updates
  ...
  Widget build(BuildContext context) {
    ...
    final lastBidTime = bidSession.lastBidTime ?? bidSession.startTime;
    final lastBidTimeNonNull = lastBidTime ?? DateTime.now(); // fallback to now
final elapsed = now.difference(lastBidTimeNonNull).inSeconds;


    final elapsed = now.difference(lastBidTime).inSeconds;

    // Determine session status dynamically
    BiddingSessionStatus status;
    if (bidSession.bidderCount > 0) {
  ...
  // Keep your existing _showBidDialog and _placeBid methods as-is
}
