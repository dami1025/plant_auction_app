...
class BiddingSessionState {
  ...
  void _initializeSession() {
    ...
    _firestoreSubscription = itemsCollection
        .doc(itemId)
        .snapshots()
  ...
  print('Firestore update: startTime=$startTime, bidCount=$bidCount, lastBidTime=$lastBidTime, status=$sessionStatus');

  if (startTime == null) return;

  final now = DateTime.now();

  // Check if session is already ended/cancelled in Firestore
  if (sessionStatus == 'ended') {
    state = state.copyWith(
      status: BiddingSessionStatus.ended,
      startTime: startTime,
      bidCount: bidCount,
  ...
  if (sessionStatus == 'cancelled') {
    state = state.copyWith(
      status: BiddingSessionStatus.cancelled,
      startTime: startTime,
      bidCount: bidCount,
      lastBidTime: lastBidTime,
    );
  ...
  // Session has started - calculate the correct end time
  DateTime endTime;
  VoidCallback timeoutAction;

  if (bidCount == 0) {
    // No bids yet → 2-minute timer from start time
    endTime = startTime.add(const Duration(minutes: 2));
    timeoutAction = _cancelSession;
    
    // If 2 minutes have already passed since start, cancel immediately
    if (now.isAfter(endTime)) {
      print('2 minutes already passed since start, cancelling session');
      _cancelSession();
      return;
    }
  } else {
    // Has bids → 60-second timer from last bid
    final lastBid = lastBidTime ?? startTime;
    endTime = lastBid.add(const Duration(seconds: 60));
    timeoutAction = _endSession;
    
    // If 60 seconds have already passed since last bid, end immediately
    if (now.isAfter(endTime)) {
      print('60 seconds already passed since last bid, ending session');
      _endSession();
      return;
    }
  }

  print('Setting active session, endTime: $endTime, timeUntilEnd: ${endTime.difference(now)}');
  ...
  _startTimer(endTime, timeoutAction);
}
...
void _startTimer(DateTime endTime, VoidCallback onTimeout) {
  ...
  print('Starting timer for duration: $duration');
  
  if (duration.isNegative || duration.inSeconds <= 0) {
    print('Timer duration is negative or zero, calling timeout immediately');
    onTimeout();
    return;
  }
  
  _sessionTimer = Timer(duration, () {
    print('Timer expired, calling timeout action');
    onTimeout();
  });
}
