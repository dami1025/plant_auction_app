...
class BiddingSessionNotifier extends StateNotifier<BiddingSessionState> {
  ...
  void _initializeSession() {
    _firestoreSubscription = FirebaseFirestore.instance
        .collection('auctions') // Adjust collection name as needed
        .doc(itemId)
        .snapshots()
  ...
  void _handleFirestoreUpdate(DocumentSnapshot doc) {
    ...
    // Determine session status based on current conditions
    if (now.isBefore(startTime)) {
    ...
    } else if (state.status == BiddingSessionStatus.notStarted) {
      // Session should start now
      _startBiddingSession(startTime, bidCount, lastBidTime);
    } else {
      ...
      // If a new bid was placed during active session, reset timer
      if (bidCount > state.bidCount && state.status == BiddingSessionStatus.active) {
        _resetSessionTimer();
      }
    }
  }
  ...
  void _startBiddingSession(DateTime startTime, int bidCount, DateTime? lastBidTime) {
    final now = DateTime.now();
  ...
  void _resetSessionTimer() {
    ...
    if (state.bidCount == 1) {
      // First bid placed - switch to 60 second timer
      final endTime = DateTime.now().add(const Duration(seconds: 60));
      state = state.copyWith(sessionEndTime: endTime);
      _startTimer(endTime, _endSession);
    } else if (state.bidCount > 1) {
      // Subsequent bid - reset 60 second timer
      final endTime = DateTime.now().add(const Duration(seconds: 60));
      state = state.copyWith(sessionEndTime: endTime);
      _startTimer(endTime, _endSession);
    }
